...
-- 2020-08-23,Krystina Letendre,Created File
--**************************************************************************--
Begin Try
	Use Master;
	If Exists(Select Name From SysDatabases Where Name = 'Assignment07DB_KrystinaLetendre')
	 Begin 
	  Alter Database [Assignment07DB_KrystinaLetendre] set Single_user With Rollback Immediate;
	  Drop Database Assignment07DB_KrystinaLetendre;
	 End
	Create Database Assignment07DB_KrystinaLetendre;
End Try
Begin Catch
	Print Error_Number();
End Catch
go
Use Assignment07DB_KrystinaLetendre;
...
go
--use function FORMAT
SELECT DISTINCT ProductName, Format(UnitPrice, 'C', 'en-US') AS 'Unit Price in USD' 
FROM vProducts
...
-- <Put Your Code Here> --
SELECT DISTINCT CategoryName, ProductName, Format(UnitPrice, 'C', 'en-US') AS 'Unit Price in USD' 
FROM vProducts JOIN vCategories ON vProducts.CategoryID = vCategories.CategoryID
...
-- <Put Your Code Here> --
SELECT ProductName, DateName(mm, InventoryDate) + ', '+ DateName(yyyy, InventoryDate) AS 'Formatted Date', vInventories.Count 
FROM vProducts JOIN vInventories ON vProducts.ProductID = vInventories.ProductID
ORDER BY ProductName, Month(InventoryDate), vInventories.Count
go
...
AS
	SELECT TOP 100000 ProductName
		, [FormattedDate] = DateName(mm, InventoryDate) + ', '+ DateName(yyyy, InventoryDate)
...
AS
	SELECT TOP 100000 CategoryName
		, DateName(mm, InventoryDate) + ', '+ DateName(yyyy, InventoryDate) AS 'Formatted Date'
		, Sum(vInventories.Count) AS 'Total Inventory Count by Category'
	FROM ((vCategories 
...
-- <Put Your Code Here> --
--Assuming prompt to set 1996 to 0 means setting January values to 0
GO 
...
AS
	SELECT TOP 100000 ProductName
		,([FormattedDate])
		,[InventoryCount]
		,[PreviousMonthCount] = IIF(Month([FormattedDate]) = 1, 0, IsNull(Lag([InventoryCount],1,0) OVER (ORDER BY ProductName, Month([FormattedDate])), 0))
...
--adding the 'order by' forced me to add the "top 10000" to the results and also the column headers.
CREATE FUNCTION dbo.fProductInventoriesWithPreviousMonthCountsWithKPIs(@KPIValue int)
 ...
 AS
	RETURN(
	 ...
	 ORDER BY Year(Cast([FormattedDate] as Date))
	);
GO
